...
func GetMenuInfo(ctx context.Context, menuIds []int) (userMenuTreeOut []*model.UserMenuTreeOut, err error) {
	...
	if !ok {
		err = gerror.New("缓存菜单数据类型错误")
		return
	}
	if err = json.Unmarshal([]byte(strVal), &tmpMenuInfo); err != nil {
		return
	}
...
// buildAuthorizeEntries builds a list of SysAuthorize entries from ID strings for a given items type.
// Each ID string is expected in the format "itemsId_isCheckAll".
// Replaces 4x duplicated loops in AddAuthorize().
func buildAuthorizeEntries(ids []string, itemsType string, roleId int) ([]*entity.SysAuthorize, error) {
	var entries []*entity.SysAuthorize
	for _, id := range ids {
		split := strings.Split(id, "_")
		if len(split) < 2 {
			return nil, gerror.Newf("%s权限参数错误", itemsType)
		}
		entries = append(entries, &entity.SysAuthorize{
			ItemsId:    gconv.Int(split[0]),
			ItemsType:  itemsType,
			RoleId:     roleId,
			IsCheckAll: gconv.Int(split[1]),
			IsDeleted:  0,
		})
	}
	return entries, nil
...
// AuthorizeIDsByType holds categorized authorize item IDs.
type AuthorizeIDsByType struct {
	MenuIDs, ButtonIDs, ColumnIDs, ApiIDs []int
}

// collectAuthorizeIDsByType categorizes authorize entries by their items type.
// Replaces duplicated ID collection in IsAllowAuthorize() and CurrentUser().
func collectAuthorizeIDsByType(items []*entity.SysAuthorize) AuthorizeIDsByType {
	var result AuthorizeIDsByType
	for _, item := range items {
		switch {
		case strings.EqualFold(item.ItemsType, consts.Menu):
			result.MenuIDs = append(result.MenuIDs, item.ItemsId)
		case strings.EqualFold(item.ItemsType, consts.Button):
			result.ButtonIDs = append(result.ButtonIDs, item.ItemsId)
		case strings.EqualFold(item.ItemsType, consts.Column):
			result.ColumnIDs = append(result.ColumnIDs, item.ItemsId)
		case strings.EqualFold(item.ItemsType, consts.Api):
			result.ApiIDs = append(result.ApiIDs, item.ItemsId)
		}
	}
	return result
}

// SecuritySwitches holds the system security switch configuration values.
type SecuritySwitches struct {
	ColumnSwitch, ButtonSwitch, ApiSwitch int
}

// getSecuritySwitches fetches the 3 security switch config values.
// Replaces 3x duplicated config-fetching pattern.
func getSecuritySwitches(ctx context.Context) (SecuritySwitches, error) {
	var sw SecuritySwitches

	colCfg, err := service.ConfigData().GetConfigByKey(ctx, "sys.column.switch")
	if err != nil {
		return sw, err
	}
	if colCfg != nil {
		sw.ColumnSwitch = gconv.Int(colCfg.ConfigValue)
	}

	btnCfg, err := service.ConfigData().GetConfigByKey(ctx, "sys.button.switch")
	if err != nil {
		return sw, err
	}
	if btnCfg != nil {
		sw.ButtonSwitch = gconv.Int(btnCfg.ConfigValue)
	}

	apiCfg, err := service.ConfigData().GetConfigByKey(ctx, "sys.api.switch")
	if err != nil {
		return sw, err
	}
	if apiCfg != nil {
		sw.ApiSwitch = gconv.Int(apiCfg.ConfigValue)
	}
