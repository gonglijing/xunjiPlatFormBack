...
func (m *Manager) GetInterface(id string) (interface{}, error) {
	...
	if client == nil {
		return nil, gerror.Newf("插件尚未启动: %s", id)
	}
...
func (m *Manager) buildPluginPath(id string) (string, error) {
	pluginType := m.Type
	if pluginType == "" {
		return "", gerror.New("插件类型为空")
	}

	basePath := m.Path
	if basePath == "" {
		basePath = pluginsFilePath
	}

	return filepath.Join(basePath, pluginType+"-"+id), nil
}
...
func (m *Manager) StartPlugin(id string) (err error) {
	if m == nil {
		return gerror.New("插件管理器为空")
	}
	if m.Plugins == nil {
		m.Plugins = map[string]*PluginInfo{}
	}
